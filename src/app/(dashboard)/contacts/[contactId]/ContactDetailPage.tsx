        ...
        <div className="grid max-w-7xl gap-8 lg:grid-cols-3">
          ...
          <section className="space-y-5">
            ...
            <div className="rounded-lg bg-ronda-bg p-4">
              <p className="text-xs uppercase text-ronda-muted">Puntuación</p>
              ...
              <p className="mt-2 text-sm text-ronda-muted">{contact.evaluation?.label || 'Todavía no hay evaluación comercial.'}</p>
            </div>
          </section>
        </div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {contact.people.map((person) => (
                  <div key={person.id} className="rounded-lg border border-ronda-border bg-ronda-bg p-4">
                    ...
                    <p className="mt-3 text-sm text-ronda-text">{person.phone || 'Sin teléfono'}</p>
                    <p className="mt-1 break-all text-sm text-ronda-muted">{person.email || 'Sin email'}</p>
                    {person.socialLinks ? <p className="mt-2 break-all text-xs text-ronda-muted">Redes: {person.socialLinks}</p> : null}
                    {person.workingHours ? <p className="mt-1 text-xs text-ronda-muted">Horario: {person.workingHours}</p> : null}
                    {person.commercialRelation ? <p className="mt-1 text-xs text-ronda-muted">Relación: {person.commercialRelation}</p> : null}
                  </div>
                ))}
              </div>
            )}
